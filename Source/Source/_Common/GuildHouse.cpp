...
bool CGuildHouseMng::ReqTenderGuildHouseList( CUser* pUser, DWORD dwGHType, OBJID objNpcId )
{
	if( IsValidObj( pUser ) == TRUE )
	{
		...
		{
			pUser->AddText( prj.GetText(TID_MMI_GHOUSE_NOTBIDTERM) );
			return FALSE;
		}
		...
		if( pTenderData != NULL )
		{
			...
			if( it != pTenderData->mapNpc.end() )
			{
				...
				{
					pUser->AddText( "Not available bid Town" );
				}
			}
			else
			{
...
bool CGuildHouseMng::IsTenderAble( CUser* pUser, OBJID objGHId, int nTenderPerin, int nTenderPenya )
{
	if( IsValidObj( pUser ) == TRUE )
	{
		...
		{
			pUser->AddText( prj.GetText(TID_MMI_GHOUSE_NOTGUILDNOTMASTER) );
			return FALSE;
		}
		...
		if( pGuildHouse != NULL )
		{
			...
			{
				pUser->AddText( prj.GetText(TID_MMI_GHOUSE_ALREADYGHOUSE) );
				return FALSE;
			}
